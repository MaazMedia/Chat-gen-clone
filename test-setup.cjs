...
try {
  ...
  const backendPkg = JSON.parse(fs.readFileSync('adk-server/package.json', 'utf8'));
...
if (fs.existsSync('adk-server/dist')) {
  ...
  console.log('❌ ADK server build output missing - run: cd adk-server && npm run build');
}
...
try {
  ...
  if (apiRoute.includes('proxyToADK') && apiRoute.includes('ADK_API_URL')) {
    console.log('✅ API proxy route configured for ADK');
  } else {
    console.log('❌ API proxy route missing ADK configuration');
  }
} catch (error) {
