...
export class WebResearcher {
  ...
  public description = 'An assistant that can search the web and fetch information from URLs';
  ...
  async invoke(message: string, context: any = {}) {
    // Simple response generation for web researcher
    if (message.toLowerCase().includes('search') || message.toLowerCase().includes('find')) {
      return {
        content: "I can help you search the web for information! Just tell me what you'd like to search for or provide a URL to fetch content from.",
        toolCalls: [],
      };
    }

    return {
      content: "Hello! I'm your Web Researcher. I can search the web for information and fetch content from URLs. What would you like me to research for you?",
      toolCalls: [],
    };
  }
