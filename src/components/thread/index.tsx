...
function convertAdkMessageToLangGraphMessage(message: Message) {
  const textContent =
...
export function Thread() {
  ...
  const { hideToolCalls, setHideToolCalls } = stream;
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    // Send message through ADK stream with content blocks
    stream.sendMessage(input.trim(), contentBlocks);
  ...
  const handleEditSubmit = (e: FormEvent) => {
    e.preventDefault();
    if (editingContent.trim().length === 0 || isLoading || !editingMessageId)
      return;

    // Update the message in place using the editMessage function
    stream.editMessage(editingMessageId, editingContent.trim());
  ...
  const hasNoAIOrToolMessages = !messages.find((m) => m.role === "assistant");
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      >
        ...
        >
          <div
            className={`${chatStarted ? "relative" : "absolute"} top-0 left-0 z-10 flex w-full items-center justify-between gap-3 p-2 pl-4`}
          >
            <div className="relative flex items-center justify-start gap-2">
              {!chatHistoryOpen && (
                <TooltipIconButton
                  tooltip={
                    chatHistoryOpen ? "Hide chat history" : "Show chat history"
                  }
                  size="lg"
            ...
            <div className="from-background to-background/0 absolute inset-x-0 top-full h-5 bg-gradient-to-b"></div>
          </div>
          {/* Chat Area */}
          <div className="flex min-h-0 flex-1 flex-col">
            <StickToBottom className="flex h-full flex-1 flex-col">
              <StickyToBottomContent
                  ...
                  <>
                    ...
                    {messages.map((message, index) => {
                      const convertedMessage =
                        convertAdkMessageToLangGraphMessage(message);

                      if (convertedMessage.role === "user") {
                        const isEditing =
                          editingMessageId === convertedMessage.id;
                          ...
                          >
                            ...
                            <div className="ml-auto flex flex-col gap-2">
                              <div className="flex flex-wrap items-end justify-end gap-2">
                                {/* Render content blocks (images/files) */}
                                {convertedMessage.contentBlocks &&
                                  convertedMessage.contentBlocks.length > 0 && (
                                    <div className="flex flex-wrap items-end justify-end gap-2">
                                      {convertedMessage.contentBlocks.map(
                              ...
                              <div className="ml-auto flex items-center gap-2 opacity-0 transition-opacity group-focus-within:opacity-100 group-hover:opacity-100">
                                <div className="ml-auto flex items-center gap-2">
                                  ...
                                  <Button
                                    ...
                                    aria-label="Edit"
                                    onClick={() => {
                                      setEditingMessageId(convertedMessage.id);
                                      setEditingContent(
                                        convertedMessage.content,
                                      );
                                    }}
                        ...
                        console.log(
                          "🔧 [Thread] Passing message to AIMessage:",
                          convertedMessage,
                        );
                        console.log(
                          "🔧 [Thread] Message tool_calls:",
                        ...
                        return (
                          ...
                          >
                            <AIMessage
                              ...
                              className="bg-muted/30 rounded-2xl border border-solid p-6"
                            />
                          </div>
                        );
                      }
                ...
                footer={
                  <div className="mx-4 mt-2 mb-4 space-y-4">
                    <ScrollToBottom className="mx-auto" />

                    <div className="sticky bottom-0 flex h-screen flex-col items-center justify-center gap-8 bg-white">
                      {!chatStarted && (
                      ...
                      <div className="bg-muted relative z-10 mx-auto mb-8 w-full max-w-3xl rounded-2xl border border-solid shadow-xs transition-all">
                        ...
                        >
                          ...
                          <div className="flex items-center gap-6 p-2 pt-4">
                            <div>
                              <div className="flex items-center space-x-2">
                                <Switch
                                  ...
                                  className="peer data-[state=checked]:bg-primary data-[state=unchecked]:bg-input focus-visible:border-ring focus-visible:ring-ring/50 inline-flex h-5 w-9 shrink-0 items-center rounded-full border-2 border-transparent shadow-xs transition-all outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50"
                                />
                                <Label
                                  data-slot="label"
                                  className="flex items-center gap-2 text-sm font-medium text-gray-600 select-none group-data-[disabled=true]:pointer-events-none group-data-[disabled=true]:opacity-50 peer-disabled:cursor-not-allowed peer-disabled:opacity-50"
                            ...
                            <Label
                              data-slot="label"
                              className="flex cursor-pointer items-center gap-2 text-sm leading-none font-medium select-none group-data-[disabled=true]:pointer-events-none group-data-[disabled=true]:opacity-50 peer-disabled:cursor-not-allowed peer-disabled:opacity-50"
                              htmlFor="file-input"
                            >
                            ...
                            <Button
                              ...
                              className="focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive bg-primary text-primary-foreground hover:bg-primary/90 ml-auto inline-flex h-9 items-center justify-center gap-2 rounded-md px-4 py-2 text-sm font-medium whitespace-nowrap shadow-md transition-all outline-none focus-visible:ring-[3px] disabled:pointer-events-none disabled:opacity-50 has-[>svg]:px-3 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4"
                            >
                              {isLoading ? (
